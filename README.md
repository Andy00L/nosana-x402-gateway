...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-129%20passing-25704F" alt="129 tests passing">
...
- **Refund ledger.** A payment that settles but fails to provision is recorded
  ...
  refund owed with its tx, and flags any reservation stuck mid-settle (a crash
  window where money may have moved unrecorded). `GET /admin/refunds` turns the
  list into ready-to-run `spl-token transfer` commands, and marking a sent
  refund closes the row; one refund tx can close only one row.
- **Rate limiting.** `GET /markets` and every `POST /rent` share one
  fixed-window budget per client address (60 requests a minute), payment header
  or not: a header alone proves nothing until verify, so it buys no exemption.
...
| --- | --- | --- | --- |
| `POST /rent` | x402 payment | 200 deployment, session, tx | 400 input, 402 payment, 404 market, 409 replay, 429 rate limit, 502 upstream, 503 capacity |
...
clone, and `bun test` runs 129 unit tests. Every command here was run against this
revision.
...
- **Rate limiting is per-address and in-memory.** The quote and rent surface is
  capped at 60 requests a minute per client address; a restart resets the
  windows and a distributed scraper with many addresses is out of scope for v1.
  Set `TRUST_PROXY=1` behind a reverse proxy or every client shares the proxy's
  budget.
